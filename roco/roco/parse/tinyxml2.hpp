...
#ifndef _ROCO_PARSE_TINYXML2_HPP_
#define _ROCO_PARSE_TINYXML2_HPP_
...
inline bool IsWhiteSpace(char p) {
	return !IsUTF8Continuation(p) && isspace(static_cast<unsigned char>(p));
}

inline const char* SkipWhiteSpace(const char* p, int* curLineNumPtr) {
	TIXMLASSERT(p);
...
inline char* SkipWhiteSpace(char* p, int* curLineNumPtr) {
...
inline bool StringEqual(const char* p, const char* q, int nChar = INT_MAX) {
	if (p == q) {
		return true;
	}
	TIXMLASSERT(p);
...
inline const char* ReadBOM(const char* p, bool* hasBOM)
{
	TIXMLASSERT(p);
	TIXMLASSERT(hasBOM);
	*hasBOM = false;
...
inline void ConvertUTF32ToUTF8(unsigned long input, char* output, int* length)
{
...
inline const char* GetCharacterRef(const char* p, char* value, int* length)
{
...
inline void ToStr(int v, char* buffer, int bufferSize)
{
	TIXML_SNPRINTF(buffer, bufferSize, "%d", v);
}

inline void ToStr(unsigned v, char* buffer, int bufferSize)
{
...
*/
inline void ToStr(float v, char* buffer, int bufferSize)
{
	TIXML_SNPRINTF(buffer, bufferSize, "%.8g", v);
}

inline void ToStr(double v, char* buffer, int bufferSize)
{
	TIXML_SNPRINTF(buffer, bufferSize, "%.17g", v);
}

inline void ToStr(int64_t v, char* buffer, int bufferSize)
{
	// horrible syntax trick to make the compiler happy about %lld
	TIXML_SNPRINTF(buffer, bufferSize, "%lld", static_cast<long long>(v));
}

inline void ToStr(uint64_t v, char* buffer, int bufferSize)
{
	// horrible syntax trick to make the compiler happy about %llu
	TIXML_SNPRINTF(buffer, bufferSize, "%llu", (long long)v);
}

inline bool	ToInt(const char* str, int* value)
{
...
inline bool ToUnsigned(const char* str, unsigned* value)
{
...
inline bool	ToBool(const char* str, bool* value)
{
	int ival = 0;
	if (ToInt(str, &ival)) {
...
inline bool	ToFloat(const char* str, float* value)
{
...
inline bool ToDouble(const char* str, double* value)
{
...
inline bool ToInt64(const char* str, int64_t* value)
{
...
inline bool ToUnsigned64(const char* str, uint64_t* value)
{
...
inline void SetBoolSerialization(const char* writeTrue, const char* writeFalse)
{
...
inline FILE* callfopen(const char* filepath, const char* mode)
{
...
public:
	...
	StrPair& operator=(const StrPair& other) = delete;	// not supported, use TransferTo()
...
public:
	...
	char* ParseText(char* p, const char* endTag, int strFlags, int* curLineNumPtr)
	{
		TIXMLASSERT(p);
...
public:
	...
	DynArray& operator=(const DynArray&) = delete;	// not supported
...
public:
	...
	MemPoolT& operator=(const MemPoolT&) = delete;	// not supported
...
class  XMLNode
{
	...
	friend void DeleteNode(XMLNode* node);
...
public:
	...
	*/
	...
	@sa Value()
	*/
	void SetValue(const char* str, bool staticMem = false)
	{
		if (staticMem) {
...
protected:
	explicit XMLNode(XMLDocument* doc) :
		_document(doc),
		_parent(0),
		_value(),
		_parseLineNum(0),
...
public:
	...
	virtual XMLNode* ShallowClone(XMLDocument* doc) const;

	virtual bool ShallowEqual(const XMLNode* compare) const
	{
		TIXMLASSERT(compare);
		const XMLText* text = compare->ToText();
		return (text && StringEqual(text->Value(), Value()));
	}
...
public:
	...
	virtual bool ShallowEqual(const XMLNode* compare) const
	{
		...
		return (comment && StringEqual(comment->Value(), Value()));
	}
...
public:
	...
	virtual bool ShallowEqual(const XMLNode* compare) const
	{
		...
		return (declaration && StringEqual(declaration->Value(), Value()));
	}
...
public:
	...
	virtual bool ShallowEqual(const XMLNode* compare) const
	{
		...
		return (unknown && StringEqual(unknown->Value(), Value()));
	}
...
public:
	...
	*/
	...
	void SetAttribute(const char* v)
	{
		_value.SetStr(v);
	}

	/// Set the attribute to value.
	void SetAttribute(int v)
	{
		char buf[BUF_SIZE];
...
public:
	...
	*/
	...
	virtual bool ShallowEqual(const XMLNode* compare) const
	{
		...
		if (other && StringEqual(other->Name(), Name())) {

			const XMLAttribute* a = FirstAttribute();
			const XMLAttribute* b = other->FirstAttribute();

			while (a && b) {
				if (!StringEqual(a->Value(), b->Value())) {
					return false;
				}
				a = a->Next();
				b = b->Next();
...
private:
	XMLElement(XMLDocument* doc) : XMLNode(doc), _closingType(OPEN), _rootAttribute(0) {}
	virtual ~XMLElement()
	{
	...
	XMLAttribute* FindOrCreateAttribute(const char* name)
	{
		...
		for (attrib = _rootAttribute;
			...
			if (StringEqual(attrib->Name(), name)) {
				break;
			}
		}
		if (!attrib) {
...
public:
	...
	XMLDocument& operator=(const XMLDocument&) = delete;	// not supported
...
public:
	...
	*/
	...
	void PushAttribute(const char* name, int v)
	{
		char buf[BUF_SIZE];
		ToStr(v, buf, BUF_SIZE);
		PushAttribute(name, buf);
	}
	void PushAttribute(const char* name, unsigned v)
	{
		char buf[BUF_SIZE];
		ToStr(v, buf, BUF_SIZE);
		PushAttribute(name, buf);
	...
	void PushAttribute(const char* name, int64_t v)
	{
		char buf[BUF_SIZE];
		ToStr(v, buf, BUF_SIZE);
		PushAttribute(name, buf);
	...
	void PushAttribute(const char* name, uint64_t v)
	{
		char buf[BUF_SIZE];
		ToStr(v, buf, BUF_SIZE);
		PushAttribute(name, buf);
	...
	void PushAttribute(const char* name, bool v)
	{
		char buf[BUF_SIZE];
		ToStr(v, buf, BUF_SIZE);
		PushAttribute(name, buf);
	...
	void PushAttribute(const char* name, double v)
	{
		char buf[BUF_SIZE];
		ToStr(v, buf, BUF_SIZE);
		PushAttribute(name, buf);
...
private:
	void PrintString(const char* p, bool restricted)	// prints out, after detecting entities.
	{
	...
	while (p && *p) {
		...
		if (ele) {
			...
			if (endTag.Empty()) {
				...
				else if (!StringEqual(endTag.GetStr(), ele->Name())) {
					mismatch = true;
				}
			}
			if (mismatch) {
...
inline const XMLElement* XMLNode::ToElementWithName(const char* name) const
{
	...
	if (StringEqual(element->Name(), name)) {
		return element;
	}
	return 0;
}
...
inline char* XMLText::ParseDeep(char* p, StrPair* parentEndTag, int* curLineNumPtr)
{
	if (this->CData()) {
...
inline char* XMLComment::ParseDeep(char* p, StrPair* parentEndTag, int* curLineNumPtr)
{
...
inline char* XMLDeclaration::ParseDeep(char* p, StrPair* parentEndTag, int* curLineNumPtr)
{
...
inline char* XMLUnknown::ParseDeep(char* p, StrPair* parentEndTag, int* curLineNumPtr)
{
...
inline XMLError XMLDocument::SaveFile(FILE* fp, bool compact/* = false*/)
{
...
inline void DeleteNode(XMLNode* node)
{
	if (node == 0) {
		return;
	}
...
#endif // _ROCO_PARSE_TINYXML2_HPP_
